...
class Human36M(Dataset):
    def __init__(self, actions, data_path, use_hg=True, is_train=True):
        ...
        if self.is_train:
            # load train data
            self.train_3d = torch.load(os.path.join(data_path, 'train_3d.pth.tar')) # z-score norm 인듯?
            self.train_2d = torch.load(os.path.join(data_path, train_2d_file)) # z-score norm
            for k2d in self.train_2d.keys():
                (sub, act, fname) = k2d
                k3d = k2d
                k3d = (sub, act, fname[:-3]) if fname.endswith('-sh') else k3d
                num_f, _ = self.train_2d[k2d].shape # (?, 32)
                assert self.train_3d[k3d].shape[0] == self.train_2d[k2d].shape[0], '(training) 3d & 2d shape not matched'
                for i in range(num_f):
                    self.train_inp.append(self.train_2d[k2d][i]) # list , shape = 32
...
Human36M(actions=actions[0], data_path=option.data_dir, use_hg=option.use_hg, is_train=True)
